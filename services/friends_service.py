...
class FriendsService:
    ...
    def import_friend_hybrid_sig_pub(
        ...
        """
        if not self.friend_exists(friend_name):
        ...
        details = self.get_friend_details(friend_name)
        if not details:
            raise FriendsServiceError(f"Friend '{friend_name}' details not found")

        secret = self.get_friend_secret(friend_name)
        x_b64 = self.get_friend_x25519_key(friend_name)
        caps = self.get_friend_capabilities(friend_name)
        pqc_b64 = None
